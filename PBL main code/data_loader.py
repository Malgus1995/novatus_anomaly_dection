...
class DataGenerator(BaseDataGenerator):
    def __init__(self, config):
        ...
        self.load_sensor_dataset(self.config['dataset'], self.config['y_scale'])

    def load_sensor_dataset(self, dataset, y_scale=5):
        data_dir = r'/content/gdrive/My Drive/Colab Notebooks/PBL main code/ai_nova/'
        ...
        # slicing training set into rolling windows
        n_training_sample = len(data['training'])
        n_training_vae = n_training_sample - self.config['l_win'] + 1
        rolling_windows = np.zeros((n_training_vae, self.config['l_win']))

        for i in range(n_training_sample - self.config['l_win'] + 1):
            rolling_windows[i] = data['training'][i:i + self.config['l_win']]

        # create VAE training and validation set
        idx_train, idx_val, self.n_train_vae, self.n_val_vae = self.separate_train_and_val_set(n_training_vae)
        ...
        for k in range(self.config['l_win']):
            n_not_overlap_wins = (n_training_sample - k) // self.config['l_win']
            ...
            for i in range(n_train_lstm):
                ...
                for j in range(self.config['l_seq']):
                    cur_seq[j] = data['training'][k + self.config['l_win'] * (j + i): k + self.config['l_win']
                                                                                      * (j + i + 1)]
    ...
    def plot_time_series(self, data, time, data_list):
        ...
        fig.subplots_adjust(hspace=.8, vspace=.4)
        axs = axs.ravel()

        for i in range(4):                       # check
            axs[i].plot(time, data[:, i])
            axs[i].set_title(data_list[i])
            axs[i].set_xlabel('time (1 min)')
            axs[i].set_xlim((np.amin(time), np.amax(time)))
