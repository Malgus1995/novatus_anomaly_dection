...
class BaseDataGenerator:
    ...
    def separate_train_and_val_set(self, n_win):
        n_train = int(np.floor((n_win * 0.9))) # 90 %
        n_val = n_win - n_train # 10 %
        idx_train = random.sample(range(n_win), n_train)
        idx_val = list(set(idx_train) ^ set(range(n_win))) # leave only duplicated elements
...
class BaseModel:
    ...
    def init_global_step(self):
        ...
        with tf.compat.v1.variable_scope('global_step'):
            ...
            self.increment_global_step_tensor = tf.compat.v1.assign(self.global_step_tensor, self.global_step_tensor + 1)

    def define_loss(self):
        with tf.name_scope('loss'):
            # minimize KL Divergence loss = maximize log likelihood
            ...
            self.KL_loss = tf.reduce_sum(KL_loss)

            # norm 1 of std of the sample-wise encoder prediction
            ...
            weighted_reconstruction_error_dataset = tf.reduce_sum(weighted_reconstruction_error_dataset)
            ...
            ls_reconstruction_error = tf.reduce_sum(
                tf.square(self.original_signal - self.decoded), [1, 2])
            self.ls_reconstruction_error = tf.reduce_sum(ls_reconstruction_error)
    ...
    def training_variables(self):
        ...
        if num_encoder is None:
            num_encoder = 0
        elif num_decoder is None:
            num_decoder = 0
        elif num_sigma2 is None:
            num_sigma2 = 0
    ...
    def compute_gradients(self):
        self.lr = tf.compat.v1.placeholder(tf.float32, [])  # origin float32, sensor signal float64
