...
class VAEmodel(BaseModel):
    ...
    def define_iterator(self):
        ...
        self.dataset = self.dataset.shuffle(buffer_size=130000, seed=self.seed) # 5 anomaly
        self.dataset = self.dataset.repeat(5000) # random value
    ...
    def build_model(self):
        init = tf.keras.initializers.glorot_normal() # tf.compat.v1.truncated_normal_initializer(stddev=0.1), tf.contrib.layers.xavier_initializer()
        with tf.compat.v1.variable_scope('encoder'):
            input_tensor = tf.expand_dims(self.original_signal, -1)
            if self.config['l_win'] == 24: # check
                conv_1 = tf.keras.layers.Conv2D(inputs=tf.pad(input_tensor, [[0, 0], [4, 4], [0, 0], [0,0]], 'SYMMETRIC'), # 'constant' (zero padding), 'reflect', 'symmetric'
                ...
                conv_2 = tf.keras.layers.Conv2D(inputs=conv_1,
                ...
                conv_3 = tf.keras.layers.Conv2D(inputs=conv_2,
                ...
                conv_4 = tf.keras.layers.Conv2D(inputs=conv_3,
            ...
            elif self.config['l_win'] == 48: # check
                conv_1 = tf.keras.layers.Conv2D(input_tensor,
                                          filters=(self.config['num_hidden_units'] / 16),
                                          kernel_size=(3, self.config['n_channel']),
                                          strides=(2, 1),
                                          padding='same',
            ...
            elif self.config['l_win'] == 144: # check
                conv_1 = tf.keras.layers.Conv2D(inputs=input_tensor,
                                          filters=(self.config['num_hidden_units'] / 16),
                                          kernel_size=(3, self.config['n_channel']),
                                          strides=(4, 1),
                                          padding='same',
            ...
            encoded_signal = tf.keras.layers.Dense(encoded_signal,
                                             units=self.config['code_size'] * 4,
                                             activation=tf.nn.leaky_relu,
                                             kernel_initializer=init)
                
            self.code_mean = tf.keras.layers.Dense(encoded_signal,
                                             units=self.config['code_size'],
                                             activation=None,
                                             kernel_initializer=init,
                                             name='code_mean')
            self.code_std_dev = tf.keras.layers.Dense(encoded_signal,
                                                units=self.config['code_size'],
                                                activation=tf.nn.relu,
                                                kernel_initializer=init,
                                                name='code_std_dev')
            self.code_std_dev += 1e-2
        ...
        with tf.compat.v1.variable_scope('decoder'):
            encoded = tf.cond(self.is_code_input, lambda: self.code_input, lambda: self.code_sample) #  return lists of output tensors
            decoded_1 = tf.keras.layers.Dense(encoded,
            ...
            if self.config['l_win'] == 24:
                decoded_2 = tf.keras.layers.Conv2D(decoded_1,
                                             filters=(self.config['num_hidden_units']),
                                             kernel_size=1,
                                             padding='same',
                                             activation=tf.nn.leaky_relu)
                ...
                decoded_3 = tf.keras.layers.Conv2D(decoded_2,
                                             filters=(self.config['num_hidden_units'] // 4),
                                             kernel_size=(3, 1),
                                             strides=1,
                                             padding='same',
                ...
                decoded_4 = tf.keras.layers.Conv2D(decoded_3,
                                             filters=(self.config['num_hidden_units'] // 8),
                                             kernel_size=(3, 1),
                                             strides=1,
                                             padding='same',
                ...
                decoded_5 = tf.keras.layers.Conv2D(decoded_4,
                                             filters=(self.config['num_hidden_units'] // 16),
                                             kernel_size=(3, 1),
                                             strides=1,
                                             padding='same',
                ...
                decoded = tf.keras.layers.Conv2D(inputs=decoded_5,
                ...
                decoded_5 = tf.nn.depth_to_space(input=decoded_5,
                                                 block_size=2)
                decoded_5 = tf.reshape(decoded_5, [-1, 144, 1, 16])   
                print("decoded_5 is: {}".format(decoded_5))
...
class lstmKerasModel:
    ...
    def plot_lstm_embedding_prediction(self, idx_test, config, model_vae, sess, data, lstm_embedding_test):
        ...
        savefig(config['result_dir'] + "lstm_seq_embedding_{}".format(idx_test))
        fig.clf()
        plt.close()